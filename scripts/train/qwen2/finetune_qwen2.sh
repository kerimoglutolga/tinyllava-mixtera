...
deepspeed --include localhost:0,1,2,3 --master_port 29501 tinyllava/train/train.py \
    ...
    --pretrained_model_path /iopsstor/scratch/cscs/tkerimog/tinyllava/models/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-pretrain \
    --output_dir /iopsstor/scratch/cscs/tkerimog/tinyllava/models/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-finetune \
    --num_train_epochs 1 \
